...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('pages.admin.dashboard.dashboard', compact(
            'stats', 'recentOrders', 'revenueMonths', 'revenueByCategory', 'lowStockProducts', 'notifications'
        ));
    }
    ...
    {
        $notifications = $this->dashboardService->getNotifications();
        $recent = array_slice(array_filter($notifications, fn ($n) => $n['unread']), 0, 3);

        // Pre-render icon SVGs so JS polling can just insert the HTML
        $icons = [
        ...
        foreach ($recent as &$n) {
            $svgPath = $icons[$n['icon']] ?? $icons['alert-triangle'];
            $n['iconHtml'] = '<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mt-0.5 shrink-0 ' . $n['icon_color'] . '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' . $svgPath . '</svg>';
        }

        return response()->json([
            'notifications' => $notifications,
            'newCount' => count(array_filter($notifications, fn ($n) => $n['unread'])),
            'recent' => $recent,
        ]);
    }
}
