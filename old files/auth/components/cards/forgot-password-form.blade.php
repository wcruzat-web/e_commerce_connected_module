{{--
    ERP MODULE: Authentication — Forgot Password Form
    COMPONENT: ForgotPasswordForm
    DESCRIPTION: Single-centered card with email input and reset link button.
...
<div class="bg-white rounded-2xl p-10 shadow-[0_0_45px_rgba(0,187,255,0.25)] text-center">
    ...
    <form method="POST" action="{{ route('forgot.password') }}" class="mt-6 text-left">
        @csrf
        @if (session('status'))
            <p class="text-xs text-green-700 bg-green-50 border border-green-200 rounded-lg p-3 mb-4">{{ session('status') }}</p>
        @endif
        ...
        <button type="submit" class="w-full mt-5 bg-[#00BBFF] hover:bg-[#00a6e0] transition-colors text-white text-sm font-semibold py-3 rounded-lg">
            Send Reset Link
        </button>
    </form>

    <a href="{{ route('login') }}" class="inline-flex items-center gap-2 text-sm font-medium text-[#00BBFF] hover:underline mt-5">
