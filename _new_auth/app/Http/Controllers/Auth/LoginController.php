...
class LoginController extends Controller
{
    public function store(Request $request): RedirectResponse
    {
        ...
        if (Auth::attempt($credentials, $request->boolean('remember'))) {
            ...
            // A password login is never a Google login — clear any stale flag.
            $request->session()->forget('auth_via');

            // Remember the entered credentials so the login form is pre-filled
            // after a logout (the user can just press Enter next time).
