        ...
        @foreach ($products as $product)
            <a href="{{ route('products.show', $product->slug) }}" class="bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-shadow overflow-hidden group block">
                ...
                <div class="bg-gray-50 h-48 flex items-center justify-center p-4 relative">
                    @if($product->badge || $product->stock === 0 || ($product->stock > 0 && $product->stock <= 5))
                        @if($product->badge)
                        ...
                        @endif
                        @if($product->stock === 0)
                            <span class="absolute top-3 right-3 text-[10px] font-semibold px-2 py-1 rounded-full bg-red-500 text-white">
                                Out of Stock
                            </span>
                        @elseif($product->stock > 0 && $product->stock <= 5)
                            <span class="absolute top-3 right-3 text-[10px] font-semibold px-2 py-1 rounded-full bg-amber-500 text-white">
                                {{ $product->stock }} left
                            </span>
                        @endif
                    @endif
