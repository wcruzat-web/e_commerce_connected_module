...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
        ...
        <div>
            ...
            @if($stock === 0)
                <div class="flex items-center gap-2 mt-6 p-4 bg-red-50 border border-red-200 rounded-xl">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-red-500 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            @else
                <form method="POST" action="{{ route('cart.add') }}" class="flex items-center gap-4 mt-6">
                    ...
                    <div class="flex items-center border border-gray-200 rounded-lg">
                        ...
                        <button type="button" onclick="const el=this.parentNode.querySelector('.qty'),inp=this.parentNode.querySelector('.qty-input');let v=parseInt(el.textContent)+1;if(v>{{ $stock }})v={{ $stock }};el.textContent=v;inp.value=v" class="w-9 h-9 flex items-center justify-center text-gray-500 hover:text-gray-700 hover:bg-gray-50 transition-colors">+</button>
                    </div>
                    @if($stock > 0 && $stock <= 5)
                        <span class="text-xs font-semibold text-amber-600">Only {{ $stock }} left</span>
                    @endif
