...
<head>
    ...
    <title>Edit Specs & Compatibility</title>
...
<body class="bg-gray-100 p-8">
    <div class="max-w-5xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <a href="/dummy/products" class="text-blue-600 hover:underline text-sm">&larr; Back to Products</a>
            </div>
            <a href="/shop" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-400">View in Shop</a>
        </div>
            ...
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                ...
                <template x-for="(spec, index) in specs" :key="index">
                    <div class="flex gap-3 mb-3 items-start">
                        ...
                        <input type="hidden" :name="'specs[' + index + '][label]'" :value="spec.label">
                        <input type="hidden" :name="'specs[' + index + '][value]'" :value="spec.value">
                        ...
                        <input type="text" x-model="spec.label" placeholder="Attribute name" class="border rounded px-3 py-2 flex-1">
                        <input type="text" x-model="spec.value" placeholder="Value" class="border rounded px-3 py-2 flex-1">
            ...
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                ...
                <template x-for="(compat, index) in compatibilities" :key="index">
                    <div class="flex gap-3 mb-3 items-start">
                        ...
                        <input type="text" x-model="compat.item_name" placeholder="Item name" class="border rounded px-3 py-2 flex-1">
    ...
    <script>
        function specsForm() {
            return {
                specs: @json($product->specifications->map(fn($s) => ['category_name' => $s->category_name, 'label' => $s->label, 'value' => $s->value])),
                compatibilities: @json($product->compatibilities->map(fn($c) => ['category_name' => $c->category_name, 'item_name' => $c->item_name])),
                addSpec() {
                    this.specs.push({ category_name: '', label: '', value: '' });
                },
                removeSpec(index) {
