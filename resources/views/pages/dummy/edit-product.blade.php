...
<body class="bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-lg">
        <div class="text-center mb-8">
            ...
            <p class="text-slate-500 text-sm mt-1">Update product details</p>
        </div>
        ...
        <form method="POST" action="/dummy/edit-product/{{ $product->id }}" class="bg-white rounded-3xl border border-gray-200/80 p-8 space-y-6 shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="sku" value="{{ old('sku', $product->sku) }}" required placeholder="e.g. RTX-4090-24G"
                           class="w-full rounded-xl border border-gray-200 bg-gray-50/50 px-4 py-3 text-sm outline-none focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100 transition-all">
                ...
                <div>
                    ...
                    <input type="text" name="brand" value="{{ old('brand', $product->brand) }}" placeholder="e.g. NVIDIA"
                           class="w-full rounded-xl border border-gray-200 bg-gray-50/50 px-4 py-3 text-sm outline-none focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100 transition-all">
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" name="price" value="{{ old('price', $product->price) }}" required placeholder="0.00"
            ...
            <div class="flex gap-3">
                <a href="/dummy/products" class="flex-1 text-center bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold text-sm px-6 py-3.5 rounded-xl transition-all">Cancel</a>
                <button type="submit" class="flex-1 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-bold text-sm px-6 py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg active:scale-[0.98]">Save Changes</button>
            </div>
        </form>

        <p class="text-center text-xs text-slate-400 mt-6">
            <a href="/dummy/products" class="text-cyan-600 hover:text-cyan-700 font-semibold underline underline-offset-2">&larr; Back to Products</a>
        </p>
    </div>
</body>
</html>
