...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-12 bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center space-x-4 pt-2">
            <form method="POST" action="{{ route('cart.add') }}" class="flex items-center space-x-4 flex-1">
                @csrf
            ...
            <form method="POST" action="{{ route('wishlist.toggle') }}" class="js-wish-form">
                @csrf
                <input type="hidden" name="product_id" value="{{ $product['id'] }}">
                <button type="submit" class="p-2 rounded-lg transition hover:bg-red-50 js-wish-btn {{ ($product['in_wishlist'] ?? false) ? 'text-red-500' : 'text-gray-400 hover:text-red-500' }}" title="{{ ($product['in_wishlist'] ?? false) ? 'Remove from Wishlist' : 'Add to Wishlist' }}">
                    <svg class="w-5 h-5 js-wish-svg" fill="{{ ($product['in_wishlist'] ?? false) ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                </button>
            </form>
        </div>
    </div>
