...
<div class="lg:col-span-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm h-fit">
    <div class="flex items-center justify-between mb-5">
        <div class="flex items-center gap-2">
            ...
            <h2 class="text-sm font-semibold text-gray-900 dark:text-white">Cart Items</h2>
            <span id="itemCountBadge" class="text-xs font-medium bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 px-2.5 py-0.5 rounded-full">{{ $cart->items->count() }} {{ $cart->items->count() === 1 ? 'item' : 'items' }}</span>
        </div>
        {{-- links to shop page --}}
            ...
            <div class="cart-item flex items-start gap-4 py-4 {{ $loop->first ? 'pt-0' : '' }}" data-item-id="{{ $item->cart_item_id }}" data-price="{{ $item->unit_price }}" data-stock="{{ $item->product->stock ?? 0 }}">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                        {{-- product brand tag --}}
                        <span class="text-[11px] font-medium bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-gray-300 px-2 py-0.5 rounded-full">{{ $item->product->brand }}</span>
                        {{-- category tag --}}
                        <span class="text-[11px] font-medium bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-gray-300 px-2 py-0.5 rounded-full">{{ $item->product->category->name ?? '' }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-4 mt-3">
                        ...
                        <div class="flex items-center border border-gray-200 dark:border-gray-600 rounded-lg overflow-hidden">
                            <form method="POST" action="{{ route('cart.update', $item->cart_item_id) }}" class="flex items-center">
                                ...
                                <button type="button" onclick="changeQty({{ $item->cart_item_id }}, -1)" class="w-7 h-7 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <line x1="5" y1="12" x2="19" y2="12"></line>
                                    </svg>
                                </button>
                                <span class="qty-value w-8 text-center text-sm font-medium text-gray-800 dark:text-gray-200">{{ $item->quantity }}</span>
                                <button type="button" onclick="changeQty({{ $item->cart_item_id }}, 1)" class="w-7 h-7 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <form method="POST" action="{{ route('cart.remove', $item->cart_item_id) }}" onsubmit="return confirm('Remove this item?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="flex items-center gap-1 text-xs font-medium text-gray-400 dark:text-gray-500 hover:text-red-500">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="text-right shrink-0">
                    <p class="line-total text-sm font-bold text-gray-900 dark:text-white">₱{{ number_format($item->subtotal, 2) }}</p>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">₱{{ number_format($item->unit_price, 2) }} each</p>
                </div>
            </div>
        @empty
            <div class="py-12 text-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-12 h-12 text-gray-200 dark:text-gray-600 mx-auto mb-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                    <circle cx="9" cy="21" r="1"></circle>
                    <circle cx="20" cy="21" r="1"></circle>
                    <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path>
                </svg>
                <p class="text-sm text-gray-400 dark:text-gray-500">Your cart is empty.</p>
