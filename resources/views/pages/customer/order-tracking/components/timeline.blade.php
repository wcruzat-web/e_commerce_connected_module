<div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-sm font-semibold text-gray-900 dark:text-white">Shipment Timeline</h2>
            ...
            @foreach ($timelineSteps as $index => $step)
                <div class="flex flex-col items-center flex-1 relative">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center z-10
                        ...
                        @else bg-gray-300 dark:bg-gray-600 text-white
                        @endif">
                        @if($step['state'] === 'done')
                    ...
                    <p class="text-xs mt-2 text-center leading-tight max-w-[80px]
                        @if($step['state'] === 'pending') text-gray-400 @else text-gray-700 dark:text-gray-300 font-medium @endif">
                        {{ $step['title'] }}
                    </p>

                    @if(!$loop->last)
                        <div class="absolute top-4 left-1/2 w-full h-0.5 -z-0
                            @if($step['state'] === 'done') bg-green-500 @else bg-gray-200 dark:bg-gray-600 @endif">
                        </div>
                    @endif
                </div>
            @endforeach
            ...
            @foreach ($timelineSteps as $step)
                <div class="flex gap-4 {{ !$loop->last ? 'pb-6' : '' }}">
                    <div class="flex flex-col items-center">
                        ...
                        @if(!$loop->last)
                            <div class="w-0.5 flex-1 mt-1 {{ $step['state'] === 'done' ? 'bg-green-500' : 'bg-gray-200 dark:bg-gray-600' }}"></div>
                        @endif
                    </div>
                    ...
                    <div class="flex-1 {{ !$loop->last ? 'pb-1' : '' }}">
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <span class="text-sm font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $step['date'] }}</span>
                        </div>
                        ...
                        <div class="flex items-start justify-between gap-2 mt-0.5">
                            <p class="text-xs text-gray-400 dark:text-gray-500">{{ $step['meta'] }}</p>
                            <span class="text-xs text-gray-400 dark:text-gray-500 whitespace-nowrap">{{ $step['time'] }}</span>
                        </div>
                    </div>
                </div>
            @endforeach
