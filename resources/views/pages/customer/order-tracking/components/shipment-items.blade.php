<div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm">
    <h2 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">Items in this Shipment</h2>
    <div class="space-y-3">
        @forelse ($order->items as $item)
            <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 rounded-xl px-4 py-3">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-white dark:bg-gray-600 border border-gray-200 dark:border-gray-600 flex items-center justify-center text-gray-500 dark:text-gray-400 shrink-0">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $item->product->name ?? 'Product #' . $item->product_id }}</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500">Qty: {{ $item->quantity }}</p>
                    </div>
                </div>
