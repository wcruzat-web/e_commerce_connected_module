...
## ERPV0.4 — Component-Split All Flat Customer Pages [AGNER]

### Structure Refactoring
All 12 flat customer pages under `resources/views/customer/` were moved to `resources/views/pages/customer/{page}/{page}.blade.php` with a `components/` subfolder. No logic changed — pure extraction.

### Pages Refactored

| Page | Components |
|------|-----------|
| `addresses/addresses` | `address-header`, `address-list` |
| `addresses/add-address` | `add-address-contact`, `add-address-details`, `add-address-footer` |
| `addresses/edit-address` | `edit-address-contact`, `edit-address-details`, `edit-address-footer` |
| `dashboard/dashboard` | `dashboard-greeting`, `dashboard-stats`, `dashboard-notifications`, `dashboard-payment-methods` |
| `orders/orders` | `orders-header`, `orders-tabs`, `orders-list`, `orders-scripts` |
| `history/history` | `history-header`, `history-list`, `history-scripts` |
| `profile/profile` | `profile-info-form`, `profile-change-password`, `profile-account-info`, `profile-addresses`, `profile-help` |
| `notifications/notifications` | `notifications-header`, `notifications-filters`, `notifications-list` |
| `wishlist/wishlist` | `wishlist-header`, `wishlist-toolbar`, `wishlist-items`, `wishlist-footer` |
| `change-password/change-password` | `change-password-form`, `change-password-google` |
| `payment-methods/payment-methods` | `payment-methods-header`, `payment-methods-list`, `payment-methods-scripts` |
| `payment-methods/add-payment` | `add-payment-header`, `add-payment-method-select`, `add-payment-card-fields`, `add-payment-gcash-fields`, `add-payment-footer`, `add-payment-scripts` |
| `settings/settings` | `settings-preferences`, `settings-notifications` (restored after accidental deletion) |

### Controller Updates
- All 13 Customer controllers updated: `view('customer.XXX')` → `view('pages.customer.XXX.XXX')`

### Shop Casing Fix
- `shop/Components/` renamed to `shop/components/` (lowercase)
- All `@include` paths updated from `Components.` to `components.`

### Old Directory Cleanup
- `resources/views/customer/` deleted (13 flat files removed)
- `resources/views/pages/customer/settings/` restored (was accidentally deleted)
